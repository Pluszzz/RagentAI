 ...
 */
package com.pluszzz.ai.ragent.infra.chat;
...
import com.pluszzz.ai.ragent.framework.convention.ChatMessage;
import com.pluszzz.ai.ragent.framework.convention.ChatRequest;
import com.pluszzz.ai.ragent.framework.trace.RagTraceNode;
import com.pluszzz.ai.ragent.infra.config.AIModelProperties;
import com.pluszzz.ai.ragent.infra.enums.ModelCapability;
import com.pluszzz.ai.ragent.infra.enums.ModelProvider;
import com.pluszzz.ai.ragent.infra.http.HttpMediaTypes;
import com.pluszzz.ai.ragent.infra.http.ModelClientErrorType;
import com.pluszzz.ai.ragent.infra.http.ModelClientException;
import com.pluszzz.ai.ragent.infra.http.ModelUrlResolver;
import com.pluszzz.ai.ragent.infra.model.ModelTarget;
...
public class DeepSeekChatClient implements ChatClient {
    ...
    public String chat(ChatRequest request, ModelTarget target) {
        ...
        try (Response response = httpClient.newCall(requestHttp).execute()) {
            if (!response.isSuccessful()) {
                ...
                log.warn("DeepSeek 同步请求失败: status={}, body={}", response.code(), body);
                throw new ModelClientException(
                        "DeepSeek 同步请求失败: HTTP " + response.code(),
            ...
            throw new ModelClientException("DeepSeek 同步请求失败: " + e.getMessage(), ModelClientErrorType.NETWORK_ERROR, null, e);
        }
    ...
    private void doStream(Call call,
        ...
        try (Response response = call.execute()) {
            if (!response.isSuccessful()) {
                String body = readBody(response.body());
                throw new ModelClientException(
                        "DeepSeek 流式请求失败: HTTP " + response.code() + " - " + body,
                        classifyStatus(response.code()),
                        response.code()
                );
            }
            ...
            if (body == null) {
                throw new ModelClientException("DeepSeek 流式响应为空", ModelClientErrorType.INVALID_RESPONSE, null);
            }
            ...
            while (!cancelled.get()) {
                ...
                try {
                    ...
                    log.warn("DeepSeek 流式响应解析失败: line={}", line, parseEx);
                }
            }
            if (!cancelled.get() && !completed) {
                throw new ModelClientException("DeepSeek 流式响应异常结束", ModelClientErrorType.INVALID_RESPONSE, null);
            }
        } catch (Exception e) {
            callback.onError(e);
        }
    ...
    private AIModelProperties.ProviderConfig requireProvider(ModelTarget target) {
        if (target == null || target.provider() == null) {
            throw new IllegalStateException("DeepSeek 提供商配置缺失");
        }
        if (target.provider().getApiKey() == null || target.provider().getApiKey().isBlank()) {
            throw new IllegalStateException("DeepSeek API密钥缺失");
        }
    ...
    private String requireModel(ModelTarget target) {
        if (target == null || target.candidate() == null || target.candidate().getModel() == null) {
            throw new IllegalStateException("DeepSeek 模型名称缺失");
        }
    ...
    private JsonObject parseJsonBody(ResponseBody body) throws IOException {
        if (body == null) {
            throw new ModelClientException("DeepSeek 响应为空", ModelClientErrorType.INVALID_RESPONSE, null);
        }
    ...
    private String extractChatContent(JsonObject respJson) {
        if (respJson == null || !respJson.has("choices")) {
            throw new ModelClientException("DeepSeek 响应缺少 choices", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonArray choices = respJson.getAsJsonArray("choices");
        if (choices == null || choices.isEmpty()) {
            throw new ModelClientException("DeepSeek 响应 choices 为空", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        JsonObject choice0 = choices.get(0).getAsJsonObject();
        if (choice0 == null || !choice0.has("message")) {
            throw new ModelClientException("DeepSeek 响应缺少 message", ModelClientErrorType.INVALID_RESPONSE, null);
        }
        ...
        String content = getJsonStringField(message, "content");
        if (content != null && !content.isBlank()) {
            return content;
        }
        String reasoning = getJsonStringField(message, "reasoning_content");
        if (reasoning != null && !reasoning.isBlank()) {
            return reasoning;
        }
        throw new ModelClientException("DeepSeek 响应缺少 content", ModelClientErrorType.INVALID_RESPONSE, null);
    }

    private String getJsonStringField(JsonObject obj, String field) {
        if (obj.has(field)) {

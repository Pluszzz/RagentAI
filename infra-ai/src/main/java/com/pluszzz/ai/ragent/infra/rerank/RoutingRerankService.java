...
public class RoutingRerankService implements RerankService {
    ...
    public List<RetrievedChunk> rerank(String query, List<RetrievedChunk> candidates, int topN) {
        List<com.pluszzz.ai.ragent.infra.model.ModelTarget> targets = selector.selectRerankCandidates();
        if (targets == null || targets.isEmpty()) {
            return candidates;
        }
        boolean allNoop = targets.stream()
                .allMatch(t -> t != null && com.pluszzz.ai.ragent.infra.enums.ModelProvider.NOOP.matches(t.candidate().getProvider()));
        if (allNoop) {
            return candidates;
        }

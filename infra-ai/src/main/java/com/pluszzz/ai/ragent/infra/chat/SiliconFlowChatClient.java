...
public class SiliconFlowChatClient implements ChatClient {
    ...
    public String chat(ChatRequest request, ModelTarget target) {
        ...
        boolean localCompat = provider.getApiKey() == null || provider.getApiKey().isBlank();
        JsonObject reqBody = buildRequestBody(request, target, false, localCompat);
        ...
        Request requestHttp = builder.build();
    ...
    public StreamCancellationHandle streamChat(ChatRequest request, StreamCallback callback, ModelTarget target) {
        ...
        // 如果没有配置 API Key，视为本地 OpenAI 兼容服务，使用同步接口模拟流式
        if (localCompat) {
            AtomicBoolean cancelled = new AtomicBoolean(false);
            CompletableFuture.runAsync(() -> {
                try {
                    String resp = chat(request, target);
                    if (!cancelled.get() && resp != null && !resp.isEmpty()) {
                        callback.onContent(resp);
                        callback.onComplete();
    ...
    private void doStream(Call call, StreamCallback callback, AtomicBoolean cancelled) {
        try (Response response = call.execute()) {
            ...
            // 有些 OpenAI 兼容服务可能不会显式发送完成标记，此时只要未被取消就视为正常结束
    ...
    private JsonObject buildRequestBody(ChatRequest request, ModelTarget target, boolean stream, boolean localCompat) {
    ...
    private Request buildStreamRequest(ChatRequest request, ModelTarget target) {
        AIModelProperties.ProviderConfig provider = requireProvider(target);
        boolean localCompat = provider.getApiKey() == null || provider.getApiKey().isBlank();
        JsonObject reqBody = buildRequestBody(request, target, true, localCompat);
        Request.Builder builder = new Request.Builder()
                .url(resolveUrl(provider, target))
                .post(RequestBody.create(reqBody.toString(), HttpMediaTypes.JSON))
                .addHeader("Content-Type", HttpMediaTypes.JSON_UTF8_HEADER)
